...
def format_decimal(value: float) -> str:
    # Format 2 desimal dalam gaya Indonesia: 1.234,56
    s = f"{value:,.2f}"
    return s.replace(",", "X").replace(".", ",").replace("X", ".")


def parse_ssa_file(path: Path) -> dict[str, dict[str, float]]:
...
def upsert_sorted_row(csv_file: Path, date_label: str, data_type: str, branch_data: dict[str, dict[str, float]]) -> None:
