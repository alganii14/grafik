...
DATE_SOURCES = [
    ("16-May", "SSA Simpanan 16 Mei 2026.csv"),
    ("17-May", "SSA Simpanan 17 Mei 2026.csv"),
    ("18-May", "SSA Simpanan 18 Mei 2026.csv"),
    ("19-May", "SSA Simpanan 19 mei 2026.csv"),
]
...
def get_code(row: list[str]) -> int | None:
    for idx in (0, 1):
        if idx >= len(row):
            continue
        token = row[idx].strip()
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if is_grand_total(row[0]) or (len(row) > 1 and is_grand_total(row[1])):
                continue
            code = get_code(row)
            if code is None or code not in BRANCH_MAPPING:
                continue
...
def main() -> None:
    ...
    print("=" * 70)
    print("Update CSV mikro 16-19 Mei 2026")
    print("=" * 70)
