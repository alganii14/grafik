...
DATE_SOURCES = [
    ("11-Jun", "SSA Simpanan_data 11 Juni 2026.csv"),
    ("12-Jun", "SSA Simpanan 12 Juni 2026.csv"),
    ("13-Jun", "SSA Simpanan 13 Juni 2026.csv"),
    ("14-Jun", "SSA Simpanan 14 Juni 2026 mentah.csv"),
    ("15-Jun", "SSA Simpanan 15 Juni 2026 Mentah.csv"),
    ("16-Jun", "SSA SIMPANAN MENTAH 16 JUNI.csv"),
]
...
def parse_ssa_file(path: Path) -> dict:
    ...
    with path.open("r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            depo = parse_number(row[value_start])
            giro = parse_number(row[value_start + 1])
            tabungan = parse_number(row[value_start + 2])
            data[branch] = {
                "depo": depo,
                "giro": giro,
                "tabungan": tabungan,
...
def duplicate_row(csv_file: Path, source_date: str, target_date: str) -> None:
    ...
    source_line = None
    for line in lines[1:]:
        if line.startswith(f"{source_date};"):
            source_line = line
            break
            
    if not source_line:
        raise ValueError(f"Baris untuk tanggal {source_date} tidak ditemukan di {csv_file.name}")
        
    new_line = source_line.replace(f"{source_date};", f"{target_date};", 1)
    body = [ln for ln in lines[1:] if not ln.startswith(f"{target_date};")]
...
def upsert_sorted_row(csv_file: Path, date_label: str, data_type: str, branch_data: dict) -> None:
    ...
    for branch in branches:
        value = branch_data.get(branch, {}).get(data_type, 0.0)
        row.append(f" {format_decimal(value)} ")

    new_line = ";".join(row) + "\n"

    body = lines[1:]
    body = [ln for ln in body if not ln.startswith(f"{date_label};")]
...
def main() -> None:
    ...
    print("=" * 70)
    print("Update CSV mikro 10 - 16 Juni 2026")
    print("=" * 70)

    # 1. Duplicate June 9 data for June 10
    print("Duplikat data 09-Jun ke 10-Jun...")
    for filename in ["depo.csv", "giro.csv", "tabungan.csv", "dpk.csv", "casa.csv"]:
        duplicate_row(base_dir / filename, "09-Jun", "10-Jun")
    print("OK 10-Jun <- 09-Jun")

    # 2. Process June 11 to 16
