"""
Script: tabrak_saldo_23agustus.py
Tujuan: Mencocokkan CIFNO dari file baseline
        DATA BASELINE DORMANT 31 DES 25 , 28 FEB_APRIL30_7MEI_17MEI.xlsx
        dengan CIFNO di file-file CSV DI319_23AGUS2026,
        lalu mengisi kolom 'saldo 23 agustus' (kolom 81).
"""
...
def main():
    ...
    excel_path = r"C:\bestline\DATA BASELINE DORMANT 31 DES 25 , 28 FEB_APRIL30_7MEI_17MEI.xlsx"
    csv_dir = r"C:\DI319\DI319_23AGUS2026"
    CIFNO_COL = 7       # Kolom G = CIFNO
    SALDO_COL = 81      # Kolom CC = saldo 23 agustus

    # ─── TAHAP 1: Membaca File Excel Baseline ────────────────────────────────
    print("=" * 60)
    print("  TABRAK SALDO 23 AGUSTUS – VIA CIFNO (BASELINE)")
    print("=" * 60)
    ...
    # Header check
    header_81 = sheet.cell(row=1, column=SALDO_COL).value
    print(f"Header kolom {SALDO_COL}: '{header_81}'")
    if header_81 != "saldo 23 agustus":
        sheet.cell(row=1, column=SALDO_COL).value = "saldo 23 agustus"
        print(f"Header kolom {SALDO_COL} diset ke: 'saldo 23 agustus'")

    # Kumpulkan target CIFNO dari kolom 7, mulai dari baris 2
    target_cifnos = set()
